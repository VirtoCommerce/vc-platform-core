...
namespace VirtoCommerce.PricingModule.Data.DynamicExpressions.Promotion.Helpers
{
    public static class PromotionEvaluationContextExtension
    {
        ...
        {
            var compareCondition = ExpressionConstants.ConditionOperation.AtLeast;
            if (isExactly)
            {
                compareCondition = ExpressionConstants.ConditionOperation.Exactly;
            }
        ...
        {
            if (compareCondition.EqualsInvariant(ExpressionConstants.ConditionOperation.Exactly))
                return context.CartPromoEntries.Where(x => x.Price * x.Quantity == lineItemTotal)
                    .ExcludeCategories(excludingCategoryIds)
                    .ExcludeProducts(excludingProductIds)
                    .Any();
            else if (compareCondition.EqualsInvariant(ExpressionConstants.ConditionOperation.AtLeast))
                return context.CartPromoEntries.Where(x => x.Price * x.Quantity >= lineItemTotal)
                    .ExcludeCategories(excludingCategoryIds)
                    .ExcludeProducts(excludingProductIds)
                    .Any();
        ...
        {
            var compareCondition = isExactly ? ExpressionConstants.ConditionOperation.Exactly : ExpressionConstants.ConditionOperation.AtLeast;
