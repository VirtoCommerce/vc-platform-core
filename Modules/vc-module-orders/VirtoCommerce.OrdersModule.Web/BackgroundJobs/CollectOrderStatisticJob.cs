    ...
    public class CollectOrderStatisticJob
    {
        ...
        public DashboardStatisticsResult CollectStatistics(DateTime start, DateTime end)
        {
            ...
            using (var repository = _repositoryFactory())
            {
                ...
                retVal.OrderCount = repository.CustomerOrders.Count(x => x.CreatedDate >= start && x.CreatedDate <= end && !x.IsCancelled);
                //avg order value
                ...
                var itemsCount = repository.CustomerOrders.Where(x => x.CreatedDate >= start && x.CreatedDate <= end)
                    .Where(x => !x.IsCancelled).Select(x => x.Items.Count);
